...
def make_pipes_vtk(nodes,full_path):
    """
    this function writes the vtk file of the pipe circuit for visualization
    """

    pipes = {key:value for key,value in nodes.items() if value.node_type != "tank"}
...
def make_pipes_vtk_transient(nodes,full_path):
    """
    this function writes the vtk file of the pipe circuit for visualization
    """

    #pipes = [node for node in nodes.values() if node.node_type != "tank"]
    pipes = {key:value for key,value in nodes.items() if value.node_type != "tank"}
